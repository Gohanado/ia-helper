...
function Build-Chrome {
    ...
    # Creer le ZIP avec contenu a la racine
    Push-Location $ChromeDir
    Compress-Archive -Path "*" -DestinationPath "..\ia-helper-chrome-v$Version.zip" -Force
    Pop-Location
...
function Build-Firefox {
    ...
    $FirefoxDir = "$DistDir\firefox-temp"
    ...
    Compress-Archive -Path "*" -DestinationPath "..\ia-helper-firefox-v$Version.zip" -Force
    Pop-Location
